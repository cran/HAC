...
\Address{
  ...
  URL: \url{https://isor.univie.ac.at/home}\\
}
...
`\code{hac}'. We provide recursive
multi-stage maximum likelihood (ML) procedure, which determine the
...
are rigorously studied in the literature. Besides, the package offers
...
reflect the variability of the parameters. A different \code{epsilon} for each aggregation of the structure is determined in the penalized ML method takin the parameter variability into account. If the
